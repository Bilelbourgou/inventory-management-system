  ...
  created: function created() {
    ...
    if (duplicateId) {
      ...
      axios.get("products/".concat(duplicateId, "/edit")).then(function (response) {
        ...
        _this8.product.type = p.type || _this8.product.type;
        _this8.product.name = p.name || "";
        _this8.product.code = p.code || "";
        _this8.product.points = p.points || "";
        _this8.product.Type_barcode = p.Type_barcode || _this8.product.Type_barcode;
        _this8.product.cost = p.cost || "";
        _this8.product.price = p.price || "";
        _this8.product.wholesale_price = p.wholesale_price || "";
        _this8.product.min_price = p.min_price || "";
        _this8.product.brand_id = p.brand_id || "";
        _this8.product.category_id = p.category_id || "";
        _this8.product.TaxNet = p.TaxNet != null ? p.TaxNet : _this8.product.TaxNet;
        _this8.product.tax_method = p.tax_method != null ? String(p.tax_method) : _this8.product.tax_method;
        _this8.product.discount_method = p.discount_method != null ? String(p.discount_method) : _this8.product.discount_method;
        _this8.product.discount = p.discount != null ? String(p.discount) : _this8.product.discount;
        _this8.product.unit_id = p.unit_id || "";
        _this8.product.unit_sale_id = p.unit_sale_id || "";
        _this8.product.unit_purchase_id = p.unit_purchase_id || "";
        _this8.product.stock_alert = p.stock_alert != null ? String(p.stock_alert) : _this8.product.stock_alert;
        _this8.product.note = p.note || "";
        _this8.product.is_imei = !!p.is_imei;
        _this8.product.not_selling = !!p.not_selling;
        _this8.product.is_featured = !!p.is_featured;

        // Warranty / Guarantee
        _this8.product.warranty_period = p.warranty_period != null ? p.warranty_period : null;
        _this8.product.warranty_unit = p.warranty_unit || _this8.product.warranty_unit;
        _this8.product.warranty_terms = p.warranty_terms || '';
        _this8.product.has_guarantee = !!p.has_guarantee;
        _this8.product.guarantee_period = p.guarantee_period != null ? p.guarantee_period : null;
        _this8.product.guarantee_unit = p.guarantee_unit || _this8.product.guarantee_unit;

        // If base unit exists, load sub-units and then set sale/purchase units
        if (_this8.product.unit_id) {
          var targetSaleId = p.unit_sale_id || "";
          var targetPurchaseId = p.unit_purchase_id || "";
          axios.get("get_sub_units_by_base?id=" + _this8.product.unit_id).then(function (_ref6) {
            var data = _ref6.data;
            _this8.units_sub = data;
            _this8.product.unit_sale_id = targetSaleId || "";
            _this8.product.unit_purchase_id = targetPurchaseId || "";
          })["catch"](function () {});
        }

        // Prefill variants (if any)
        if (Array.isArray(p.ProductVariant) && p.ProductVariant.length) {
          _this8.variants = p.ProductVariant.map(function (v, idx) {
            return {
        ...
        if (_this8.product.type === 'is_combo' && Array.isArray(response.data.materiels)) {
          _this8.materiels = response.data.materiels.slice();
        }
      })["catch"](function () {
